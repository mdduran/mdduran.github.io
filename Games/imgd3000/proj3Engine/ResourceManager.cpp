...
int ResourceManager::loadSound(std::string filename, std::string label){
	...
	if (sound_count == MAX_SOUNDS){
		log_manager.writeLog("Sound array is full");
		return -1;
	}
	if (sound[sound_count].loadSound(filename) == -1){
		log_manager.writeLog("Unable to load from file");
		return -1;
	}
	...
	sound_count++;
	log_manager.writeLog("Sound loaded succesfully");
	return 0;
}
...
int ResourceManager::unloadSound(std::string label){
	...
	for (int i = 0; i <= sound_count-1; i++){
		...
		log_manager.writeLog("Could not unload the sound");
		return -1;
...
Sound *ResourceManager::getSound(std::string label){
	...
	for (int i = 0; i <= sound_count-1; i++){
		...
		log_manager.writeLog("Could not get the sound");
		return NULL;
}
...
int ResourceManager::loadMusic(std::string filename, std::string label){
	...
	if (music_count == MAX_MUSICS){
		log_manager.writeLog("Music array is full");
		return -1;
	}
	...
	if (music[music_count].getLabel().compare("")){
		log_manager.writeLog("Cannot open a blank file");
		return -1;
	}
...
int ResourceManager::unloadMusic(std::string label){
	...
	log_manager.writeLog("Could not unload the music");
	return -1;
}
...
Music *ResourceManager::getMusic(std::string label){
	...
	log_manager.writeLog("Could not get the Music");
	return NULL;
}